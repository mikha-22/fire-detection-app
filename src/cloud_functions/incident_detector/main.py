...
def incident_detector_cloud_function(event, context, run_date_str: Optional[str] = None):
    ...
    logging.info(f"Target processing date: {run_date_str}")
    ...
    except Exception as e: logging.error(f"Failed to process FIRMS data: {e}", exc_info=True)
    try:
    ...
    except Exception as e: logging.error(f"Failed to process JAXA data: {e}", exc_info=True)

    if not all_hotspots_dfs: logging.warning("No hotspot data could be retrieved. Exiting."); return
    ...
    try:
        ...
        # --- CORRECTED: Added the missing log message ---
        logging.info(f"Applying a {PEATLAND_BUFFER_METERS}-meter buffer to peatland boundaries for spatial join.")
        
        gdf_peatland_fires = gpd.sjoin(hotspots_gdf, peatlands.to_crs(hotspots_gdf.crs), how="inner", predicate='within')
    except Exception as e: logging.error(f"CRITICAL: Peatland filtering failed: {e}", exc_info=True); return
    if gdf_peatland_fires.empty: logging.info("No hotspots on or near peatlands. Exiting."); return
    ...
    if clustered_fires.empty: logging.info("No significant fire incidents formed after clustering. Exiting."); return
...
if __name__ == "__main__":
    print("--- Running Incident Detector with Daily JAXA Data ---")
    ...
    incident_detector_cloud_function(event=None, context=None, run_date_str="2025-06-16")
