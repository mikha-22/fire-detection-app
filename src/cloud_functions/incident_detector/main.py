...
def assess_fire_severity(point_count: int, weather: dict, air_quality: dict) -> dict:
    ...
    if weather and not weather.get("error"):
        if weather.get("relative_humidity_percent", 100) < 40: severity_score += 2; factors.append("Low humidity (<40%)")
        if weather.get("wind_speed_kmh", 0) > 20: severity_score += 1; factors.append("High wind speed (>20km/h)")
    ...
    return {"overall_severity": overall, "severity_score": severity_score, "contributing_factors": factors, "details": {"hotspot_count": point_count, "weather_severity": "high" if weather.get("relative_humidity_percent", 100) < 30 else "low", "air_quality_severity": aq_severity}}
...
def incident_detector_cloud_function(event, context, run_date_str: Optional[str] = None):
    ...
    try:
        ...
        _log_json("INFO", f"Applying a {PEATLAND_BUFFER_METERS}-meter buffer to peatland boundaries")
        gdf_peatland_fires = gpd.sjoin(hotspots_gdf, peatlands.to_crs(hotspots_gdf.crs), how="inner", predicate='within')
    except Exception as e: _log_json("CRITICAL", f"Peatland filtering failed: {e}"); return
    ...
    jsonl_content = "\n".join([json.dumps(incident, indent=2) for incident in all_incidents])
    ...
    message_json = json.dumps({"status": "incidents_detected", "incident_count": len(all_incidents), "run_date": run_date_str, "severity_summary": severity_counts})
